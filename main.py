...
def init_db(db_path):
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS osoby 
                 (id INTEGER PRIMARY KEY AUTOINCREMENT, 
                  ...
                  adres TEXT, info TEXT)''')
    
    # --- MIGRACJA: Dodawanie kolumny pojazdy, jeśli jej nie ma ---
    try:
...
def main(page: ft.Page):
    ...
    page.title = "OSK System v2.6"
    page.theme_mode = ft.ThemeMode.DARK
    page.dark_theme = ft.Theme(color_scheme_seed=ft.colors.BLUE_ACCENT)
    page.padding = 0
    page.window_width = 400 # Orientacyjnie dla desktopa
    page.update()
    ...
    try:
        ...
        search_bar = ft.TextField(
            ...
            on_change=lambda _: odswiez_liste(),
        ...
        def wybierz_osobe(id_osoby):
            ...
            if r:
                ...
                # Obsługa nowej kolumny pojazdy (indeks 6)
        ...
        def zapisz_osobe(e):
            if not txt_nazwisko.value:
                page.snack_bar = ft.SnackBar(ft.Text("Nazwisko jest wymagane!"))
            ...
            page.snack_bar = ft.SnackBar(ft.Text("Dane zapisane pomyślnie"))
            page.snack_bar.open = True
            page.update()
        ...
        def potwierdz_usun(e):
            ...
            dlg = ft.AlertDialog(
                title=ft.Text("Usuwanie akt"),
                content=ft.Text("Czy na pewno chcesz trwale usunąć ten rekord?"),
                actions=[
        ...
        tabs = ft.Tabs(
            ...
            tabs=[
                ft.Tab(text="REJESTR", icon=ft.icons.STORAGE, content=widok_szukaj),
        ...
        page.add(
            ft.Container(
                ...
                content=ft.Text("OSK SYSTEM 2.6", size=20, weight="bold", color=ft.colors.BLUE_ACCENT, text_align=ft.TextAlign.CENTER),
        ...
        # Ten blok wyświetli błąd nawet jeśli interfejs nie wstanie
        page.clean()
        page.add(
            ft.Container(
                padding=20,
                content=ft.Column([
                    ft.Text("KRYTYCZNY BŁĄD URUCHOMIENIA", color="red", weight="bold", size=20),
                    ft.Text(str(ex), selectable=True),
                    ft.ElevatedButton("SKOPIUJ BŁĄD", on_click=lambda _: page.set_clipboard(str(ex)))
                ])
            )
        )
        page.update()
